...
```js
import 'jspschy-vue/src/index.css'
```

or add below to style:
...
## Reference
### Props
sPsych components accept a property called [options](https://www.jspsych.org/v7/reference/jspsych/#parameters)which can be used to configure the JsPsych object. The distinction is as follows:
...
- When both `start` and `options.on_start` are defined, `options.on_start` will execute as the component's `onMounted` hooks. The same applies to `finish`.
